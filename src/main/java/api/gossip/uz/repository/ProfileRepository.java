...
public interface ProfileRepository extends JpaRepository<ProfileEntity, Integer>, PagingAndSortingRepository<ProfileEntity, Integer> {
    Optional<ProfileEntity> findByUsernameAndVisibleTrue(final String username);

    Optional<ProfileEntity> findByIdAndVisibleTrue(final Integer id);

    @Modifying
    @Transactional
    @Query("update ProfileEntity set status= :status where id= :id")
    void changeStatus(@Param("id") final Integer id, @Param("status") final GeneralStatus status);

    @Modifying
    @Transactional
    @Query("update ProfileEntity set password =:password where id =:id")
    void updatePassword(@Param("id") final Integer id, @Param("password") final String password);

    @Modifying
    @Transactional
    @Query("update ProfileEntity set name =:name where id =:id")
    void updateDetail(@Param("id") Integer id, @Param("name") final String name);

    @Modifying
    @Transactional
    @Query("update ProfileEntity set tempUsername =:tempUsername where id =:id")
    void updateTempUsername(@Param("id") final Integer id, @Param("tempUsername") final String tempUsername);

    @Modifying
    @Transactional
    @Query("update ProfileEntity set username =:username where id =:id")
    void updateUsername(@Param("id") final Integer id, @Param("username") final String username);

    @Modifying
    @Transactional
    @Query("update ProfileEntity set photoId =:photoId where id =:id")
    void updatePhoto(@Param("id") final Integer id, @Param("photoId") final String photoId);
    ...
    Page<ProfileDetailMapper> filter(@Param("query") final String query, PageRequest pageRequest);
